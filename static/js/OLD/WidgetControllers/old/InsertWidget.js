...
if (!window.PHYLOBOX) (function() {
    ...
    PHYLOBOX = {
        ...
        loadTrees : function() {
            ...
            //see if we are calling the fullscreen version
            if ($('div.phylobox_embed_parent').attr('height')==0){
                $("div.phylobox_embed_parent").attr('width', $(window).width());
                $("div.phylobox_embed_parent").attr('height', $(window).height());
                fullscreen = true;
                $.ajaxSetup({async: false});
                $.getScript("/static/javascript/WidgetControllers/latest/FullScreen.js");
            ...
            $('div.phylobox_embed_parent').each(function(){
                ...
                $.getJSON("http://"+HOST+"/lookup"+"?k="+appId+"&callback=?",function(phylojson){
                    ...
                    var tree = phylojson.tree;
                    self.globals.trees[canvasId]['tree'] = {};
                    self.globals.trees[canvasId]['environment'] = phylojson['environment'];
                    self.globals.trees[canvasId]['environment']['WIDTH'] = pbWIDTH;
                    self.globals.trees[canvasId]['environment']['HEIGHT'] = pbHEIGHT;
                    self.globals.trees[canvasId]['environment']['maxDepth'] = 0;
                    self.globals.trees[canvasId]['environment']['nodeDepths'] = {};
                    self.globals.trees[canvasId]['environment']['stepX'] = 0;
                    self.globals.trees[canvasId]['environment']['stepY'] = 0;
                    self.globals.trees[canvasId]['environment']['key'] = appId;
                    self.globals.trees[canvasId]['environment']['fullscreen'] = fullscreen;
                    self.globals.trees[canvasId]['environment']['leafnodes'] = [];
                    self.globals.trees[canvasId]['environment']['NODE_RADIUS'] = 5;
                    for (var i in tree){
                        var id = tree[i]["id"];
                        self.globals.trees[canvasId]['tree'][id] = tree[i];
                        if (!tree[i]['children']) {
                            self.globals.trees[canvasId]['environment']['leafnodes'].push(tree[i]["id"]);
                        }
                    }
                    plotDendrogram(canvasId);
                });
