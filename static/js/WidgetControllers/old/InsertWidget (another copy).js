    ...
    PHYLOBOX = {
        ...
        loadTrees : function() {
            ...
            $('div.phylobox_embed_parent').each(function(){
                ...
                self.globals.trees[canvasId]['WIDTH'] = pbWIDTH;
                self.globals.trees[canvasId]['HEIGHT'] = pbHEIGHT;
                self.globals.trees[canvasId]['maxDepth'] = 0;
                self.globals.trees[canvasId]['nodeDepths'] = {};
                self.globals.trees[canvasId]['stepX'] = 0;
                self.globals.trees[canvasId]['stepY'] = 0;
                self.globals.trees[canvasId]['key'] = appId;
                self.globals.trees[canvasId]['fullscreen'] = fullscreen;
                
                //lookup method can also take a callback variable, get or post
                //Full URL: http://phylobox.appspot.com/lookup
                
                var url = "http://phylobox.appspot.com/lookup";
                ...
                $.getJSON(url,function(nodes){
                    tmpNodes = nodes['Nodes'];
                    tmpRootId = nodes['rootId'];
                    tmpLeafNodes = nodes['leafNodes'];
                    self.globals.trees[canvasId]['maxDepth'] = nodes['maxDepth'];
                    self.globals.trees[canvasId]['nodeDepths'] = nodes['nodeDepths'];
                    self.globals.trees[canvasId]['stepX'] = nodes['stepX'];
                    self.globals.trees[canvasId]['stepY'] = nodes['stepY'];
                    
                    self.globals.trees[canvasId]['rootId'] = tmpRootId;
                    self.globals.trees[canvasId]['Nodes'] = tmpNodes;
                    self.globals.trees[canvasId]['leafNodes'] = tmpLeafNodes;
