...
PHYLOBOX.renderPhylo = function(appId, mode, parentNodeId) {
    ...
    for (var x=0; x < PHYLOBOX.externalLibs.length; x++){
        ...
        if (PHYLOBOX.externalLibs.length == PHYLOBOX.globals.numberLibsLoaded){
...
function readTreeDivs(){
    $(".phylobox_parent").each(function(){
        pbHEIGHT = this.style.height;
        alert(pbHEIGHT);
        pbWIDTH = this.style.width;
        var parentId = this.id;
        var tmp = parentId.split("_");
        var appId = tmp[1];
        PHYLOBOX.loadTree(appId,parentId);
    });
}
    
function BlogspotPhylobox(){
