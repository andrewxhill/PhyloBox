...
function drawDendrogram(ctx,w,h){
    setInterval(function(){
        if(phylojson['environment']['color']){
            ...
            $('body').css('background',"#"+phylojson['environment']['color']);
        } else {
        ...
        function drawNode(i) {
                ...
                if (lastClick['x'] && lastClick['y']){
                    if (lastClick['x']<Nodes[i][xindex]*w+phylojson['environment']['radius']+4 && Nodes[i][xindex]*w-phylojson['environment']['radius'] < lastClick['x']-4){
                        if (lastClick['y']<Nodes[i][yindex]*h+phylojson['environment']['radius']+4 && Nodes[i][yindex]*h-phylojson['environment']['radius'] < lastClick['y']-4){
            ...
            //}
            if (Nodes[id]['visibility'] && Nodes[id]['children']){
                for (var tmp in Nodes[id]['children']){
                    drawNode(Nodes[id]['children'][tmp]['id']);
                }
            }
        }
    drawNode(rootId);
